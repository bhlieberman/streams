...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        StateInfo cardinalState = states.stream()
          .filter(v -> Objects.equals(v.getStateBird(), "cardinal"))
          .toList()
          .get(0);
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(state -> {
          return state.getStateMotto().split("\s").length == 1;
        });
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream()
          ...
          .collect(averagingDouble(CapitalInfo::getAverageYearlyPrecipitationInInches));
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream()
          .map(StateInfo::getCapital).mapToInt(CapitalInfo::getAverageYearlyPrecipitationInInches).sum();
        ...
        List<String> stateTreesSortedAscending = states.stream()
          .map(StateInfo::getStateTree).sorted(String::compareTo).toList();
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream()
          .map(StateInfo::getStateBird)
          .filter(bird -> !bird.equals("mockingbird"))
          .distinct()
          .toList();
        ...
        List<String> allStateAndCapitalNames = states.stream()
          .flatMap(state -> {
            String joined = state.getStateName() + state.getCapital();
        ...
        List<List<String>> allStateAndCapitalNamesTogetherAsLists = states.stream()
          .map(state -> List.of(
            state.getStateName(), state.getCapital().getCapitalName()
          )).toList();
